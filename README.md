...
| `@지라봇 sync` | Jira → DB 수동 동기화 |
| `@지라봇 완료` | 이슈 스레드에서 → Jira 완료 처리 |

### 자연어 입력 (AI 분류 → Jira 이슈 생성)

```
@지라봇 로그인 페이지에서 500 에러 발생     → 버그로 등록
@지라봇 다크모드 지원해주세요               → 기능 요청으로 등록
```
...
| 명령 | 설명 |
|---|---|
| `@지라봇 하위작업 <내용>` | 하위작업 생성 (Sonnet이 제목/SP 추정) |
| `@지라봇 댓글 <내용>` | Jira 이슈에 코멘트 추가 |
| `@지라봇 수정 <내용>` | Jira 설명에 내용 추가 (append) |
| `@지라봇 완료` | Jira 상태 완료로 전환 |
...
```
[채널]
나: @지라봇 결제 완료 후 금액이 0원으로 표시됩니다
봇: ✅ Jira 이슈가 등록되었습니다!
    [SLAC-15] 결제 금액 0원 표시 버그
    분류: BUG | Story Point: 5

    [스레드에서]
    나: @지라봇 하위작업 프론트엔드 금액 표시 로직 수정
    봇: ✅ 하위작업 생성: SLAC-16 (상위: SLAC-15)

    나: @지라봇 댓글 재현 조건: 카드 결제만 해당
...
변경 후 Spring Boot 재시작 + `@지라봇 sync` 필요.
