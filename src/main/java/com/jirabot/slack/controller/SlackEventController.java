...
public class SlackEventController {
    ...
    private static final String HELP_TEXT = """
            ...
            *스레드 액션 (이슈 스레드에서 댓글로 사용):*
              `@지라봇 하위작업 <내용>` — 하위작업 생성
              `@지라봇 댓글 <내용>` — Jira 코멘트 추가
              `@지라봇 수정 <내용>` — Jira 설명에 내용 추가
    ...
    // STUDY: 스레드 액션 — 부모 이슈가 있는 스레드에서 댓글로 @지라봇 호출 시.
    ...
    private void handleWithIntent(SlackEventInner event, String cleaned) {
        new Thread(() -> {
            ...
            switch (intent.intent()) {
                ...
                case "search" ->
                        replyThread(event, ":mag: 검색 기능은 준비 중입니다. `@지라봇 help`를 확인해주세요.");
                case "statistics" ->
        ...
        if (event.thread_ts() == null) {
            if (event.channel() != null && event.ts() != null) {
                slackNotifier.postThreadReply(event.channel(), event.ts(),
                        "이슈 생성 스레드에서 댓글로 `@지라봇 완료`를 사용해주세요.");
            }
            return;
        }
