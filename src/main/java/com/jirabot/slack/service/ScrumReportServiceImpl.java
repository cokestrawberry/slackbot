...
public class ScrumReportServiceImpl implements ScrumReportService {
    ...
    @Override
    public CompletableFuture<List<String>> generateReport() {
        try {
            List<IssueEntity> allIssues = issueRepository.findAll();
            if (allIssues.isEmpty()) {
                return CompletableFuture.completedFuture(
                        List.of("DB에 이슈가 없습니다. `@봇더지라 sync`로 동기화해주세요."));
            }

            // STUDY: 활성 스프린트와 백로그를 분리해 각각 별도 Slack 메시지로 보낸다.
            ...
            if (!sprintIssues.isEmpty()) {
                messages.add(String.format(":clipboard: *스프린트 리포트 — '%s'*\n\n", activeSprintName)
                        + formatScopeBody(sprintIssues));
            }
            if (!backlogIssues.isEmpty()) {
                messages.add(":clipboard: *백로그 리포트*\n\n" + formatScopeBody(backlogIssues));
            }
            if (messages.isEmpty()) {
