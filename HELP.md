...
Slack에서 `@봇더지라 help`를 입력하면 같은 내용이 채널에 표시됩니다. 운영자/개발자 참조용으로
...
| 명령 | 동작 |
|---|---|
| `@봇더지라 help` (`도움말`) | 이 도움말을 스레드 답글로 표시 |
| `@봇더지라 scrum` (`스크럼`) | 스프린트 일일 리포트 채널 게시 |
| `@봇더지라 내작업` (`my`) | 호출자의 진행 중인 작업 조회 |
| `@봇더지라 작업 <이름>` | 지정한 팀원의 진행 중인 작업 조회 |
| `@봇더지라 등록 <Jira 사용자명>` | 호출자의 Slack ↔ Jira 계정 매핑 등록 |
| `@봇더지라 sync` (`동기화`) | Jira → 로컬 DB 수동 동기화 |
| `@봇더지라 완료` (`done`) | 이슈 스레드 안에서 Jira 상태를 "완료"로 전환 |

## 스레드 액션 (이슈 생성 스레드에서 댓글로 사용)

| 명령 | 동작 |
|---|---|
| `@봇더지라 하위작업 <내용>` | Sonnet 분류 → Jira 하위 작업 생성 |
| `@봇더지라 댓글 <내용>` | Jira 이슈에 코멘트 추가 |
| `@봇더지라 수정 <내용>` | Jira 설명 본문에 내용 추가(append) |
